...
export default class PlayerTrend extends Component {
  ...
  calculateLog(data, indicator) {
    ...
    let high = parseInt(data[0].teams[0][indicator], 10);
    let low = parseInt(data[0].teams[0][indicator], 10);
    let highDate = data[0].seasonYear;
    let lowDate = data[0].seasonYear;
    let sum = 0;

    let value;
    data.forEach((d, index) => {
      value = parseInt(d.teams[0][indicator], 10);
      console.log('ARR', value);
      sum += value;
      if (value < low) {
        low = value;
        lowDate = data[index].seasonYear;
      } else if (value > high) {
        high = value;
        highDate = data[index].seasonYear;
      }
    });
    ...
    return data.map((value, index) => {
      return (
        <PlayerTrendBarItem
          ...
          date={this.props.data[index].seasonYear}
    ...
    return (
      <View style={styles.container}>
        ...
        <ScrollView
          ...
          {this.renderBars(
            data.map(d => d.teams[0].plusMinus),
            footData.high,
            footData.low,
            color,
          )}
        ...
        <View style={styles.summary}>
          ...
          <View style={styles.sumRight}>
            <View style={styles.sumPolarItem}>
              ...
              <Text style={styles.sumPolarLabel}>
                {'in ' + footData.highDate}
              </Text>
            </View>
            <View style={styles.sumPolarItem}>
